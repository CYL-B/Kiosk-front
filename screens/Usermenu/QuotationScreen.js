    ...
    if (toggleValue == false) {
        ...
        var devis = quotations.map((quotation, i)=>{
            if(quotation.status == "requested"){
                ...
                <Card.Image style={{borderRadius:100, width:60, height:60}}source={{uri:quotation.logo}}></Card.Image>
                        <Text>{quotation.name}</Text>
                        <Text><Badge badgeStyle={{backgroundColor:"#808080"}} />Devis en cours</Text>
                    
                
                    
                    <Card.Divider style={{backgroundColor: "#FAF0E6", alignItems:"flex-end"}}>
                    <Button
                    title="Contacter"
            ...
            </Card>)}else if(quotation.status == "accepted"){return(
            <Card>
                <Card.Title>{quotation.offer}</Card.Title>
            ...
            </Card>)} else if(quotation.status == "paid"){
            return(<Card>
                <Card.Title>{quotation.offer}</Card.Title>
